...
Required.add_argument(
	...
	metavar = '"str"', help = "Audio file path. {0}Accepts MP3/FLAC/OGG".format(Styles.OK)
)
...
Required.add_argument(
	...
	metavar = '"str"',
	help = '{1}Static{2} image file path. {0}If none, tries to get an embedded front cover art from "-a"{2}'.format(
...
Optional.add_argument(
	...
	metavar = "[range(96, 257)]k",
	help = "{0}Video audio bitrate{4}, in {2}kb/s{4}. If {2}> 256{4}, the {3}experimental{4} lossless {1}FLAC{4} codec is used".format(
...
Switch.add_argument(
	"-rt", "--resize-thumbnail", action = ArgParseBool(not Config["Media"]["Image"]["Thumbnail"]["Resize"]),
...
Switch.add_argument(
	"-nd", "--no-description", action = ArgParseBool(not Config["Media"]["Description"]["Generate"]),
	help = "Doesn't generate the description file"
)

Switch.add_argument(
	"-nb", "--no-bell", action = ArgParseBool(not Config["Settings"]["Logs"]["Sound"]),
	help = 'Doesn\'t print the {0}"BEL"{1} character at ending'.format(Styles.Meta_Info_2, Styles.Reset)
)

Switch.add_argument(
	"-nod", "--no-open-directory",
	action = ArgParseBool(not Config["Settings"]["Directory"]["Open"]),
	help = "Doesn't open directory after creating file"
)
