...
def __GTL(Log: str, Info: str = "", Tabs: int = 3, End: str = "\n"):
	if Info:
		Log += "\t" * Tabs + str(Info)
	return print(Log, end = End)
...
if Cover.mode != "L":
	...
	for Grayscale in range(255):
		...
		if Grayscale in Colors:
			...
			__GO += 1
	__GTL("Removed monochrome colors", f"{__GO} found", 2)

# Remove duplicates
__RD = len(Colors)
Colors = list(set(Colors))
__RD = __RD - len(Colors)
__GTL("Removed duplicated colors", f"{__RD} removed", 2)
...
__GTL("Sorted colors", f"{__CBS} changed its place", 4)
...
__GTL(
	...
	fg(*RGB(__BgCol[0])) + ThumbChar * 3 + Styles.Flaw
)
